...
def main():
    ...
    # -------------------------------------------------------------
    # Check 6: Gate 2 In-App Updater & Version Synchronization
    # -------------------------------------------------------------
    print("\n--- Check 6: Gate 2 In-App Updater and Version Synchronization ---")
    updater_failures = []
    
    # 6a. version.json RFC 8259 strict compliance
    vj_path = root / "version.json"
    if not vj_path.exists():
        updater_failures.append("version.json does not exist!")
    else:
        with open(vj_path, "rb") as f:
            vj_raw = f.read()
        if vj_raw.startswith(b"\xef\xbb\xbf"):
            updater_failures.append("version.json starts with UTF-8 BOM (0xEF 0xBB 0xBF)!")
        else:
            print("Confirmed version.json has NO UTF-8 BOM")
        
        import json
        try:
            vj_data = json.loads(vj_raw.decode("utf-8"))
            print(f"Parsed version.json: version={vj_data.get('version')}, code={vj_data.get('version_code')}")
            if vj_data.get("version") != "8.6.0":
                updater_failures.append(f"version.json version expected '8.6.0', got '{vj_data.get('version')}'")
            if vj_data.get("version_code") != 86:
                updater_failures.append(f"version.json version_code expected 86, got {vj_data.get('version_code')}")
            if "releases/download/v8.6/app-release.apk" not in vj_data.get("apk_url", ""):
                updater_failures.append("version.json apk_url does not target release tag v8.6")
        except Exception as e:
            updater_failures.append(f"Failed to parse version.json with strict RFC 8259 decoder: {e}")

    # 6b. AppUpdateManager
    aum_path = root / "lib" / "core" / "services" / "app_update_manager.dart"
    if not aum_path.exists():
        updater_failures.append("lib/core/services/app_update_manager.dart does not exist!")
    else:
        with open(aum_path, "r", encoding="utf-8") as f:
            aum_text = f.read()
        m_aum_v = re.search(r"static const String currentVersion = '([^']+)';", aum_text)
        m_aum_c = re.search(r"static const int currentVersionCode = (\d+);", aum_text)
        aum_v = m_aum_v.group(1) if m_aum_v else None
        aum_c = int(m_aum_c.group(1)) if m_aum_c else None
        print(f"AppUpdateManager: currentVersion={aum_v}, currentVersionCode={aum_c}")
        if aum_v != "8.6.0":
            updater_failures.append(f"AppUpdateManager currentVersion expected '8.6.0', got '{aum_v}'")
        if aum_c != 86:
            updater_failures.append(f"AppUpdateManager currentVersionCode expected 86, got {aum_c}")

        # 6c. Check update evaluation logic
        if aum_c is not None and "version_code" in vj_data:
            target_code = vj_data["version_code"]
            will_prompt = target_code > aum_c
            print(f"Update evaluation: target_code ({target_code}) > current_code ({aum_c}) -> {will_prompt}")
            if will_prompt:
                updater_failures.append("AppUpdateManager will trigger a false-positive update modal on launch!")
            else:
                print("Confirmed AppUpdateManager will NOT trigger false-positive update dialog.")

    # 6d. pubspec.yaml alignment
    pub_path = root / "pubspec.yaml"
    with open(pub_path, "r", encoding="utf-8") as f:
        pub_text = f.read()
    m_pub = re.search(r"^version:\s*([0-9.]+)\+([0-9]+)", pub_text, re.MULTILINE)
    if not m_pub or m_pub.group(1) != "8.6.0" or int(m_pub.group(2)) != 86:
        updater_failures.append(f"pubspec.yaml version expected '8.6.0+86', got '{m_pub.groups() if m_pub else None}'")
    else:
        print(f"Confirmed pubspec.yaml version is {m_pub.group(1)}+{m_pub.group(2)}")

    # 6e. build.gradle alignment
    if "versionCode 86" not in gradle_content or 'versionName "8.6.0"' not in gradle_content:
        updater_failures.append("android/app/build.gradle version code or name not aligned to 86 / 8.6.0")
    else:
        print("Confirmed build.gradle version code is 86 and name is '8.6.0'")

    failures.extend(updater_failures)
    checks.append(("Gate 2 In-App Updater and Version Synchronization", len(updater_failures) == 0))
